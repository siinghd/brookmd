package io.github.siinghd.fluxmd
...
/**
 * JVM-host wire-contract goldens for the generated uniffi Kotlin bindings.
 *
 * These mirror `crates/flux-md-ffi/tests/wire_golden.rs` one-to-one: the same
 * fixed document, the same chunking, and the SAME golden strings (copied
 * verbatim). They prove the Kotlin binding drives the native parser to
 * byte-identical wire (WIRE.md v1.0.0) — the FFI boundary the JS/WASM renderer
 * also decodes.
 *
 * The native library is located by JNA via `-Djna.library.path` (see
 * host-test/build.gradle.kts); CI points it at the host `libflux_md_ffi.so`.
 *
...
class WireGoldenTest {
    ...
    /** A [FluxConfig] with every setter at `StreamParser`'s library default plus the block-data toggle. */
    private fun libDefaultConfig(blockData: Boolean) = FluxConfig(
        gfmAutolinks = false,
        gfmAlerts = false,
        gfmTagfilter = false,
        gfmFootnotes = false,
    ...
    private fun stream(session: FluxSession): List<String> =
        chunks.map { session.append(it) } + session.finalize()

    @Test
    fun goldenWireDefaultBare() {
        val got = stream(FluxSession())
        assertEquals(offAppend0, got[0], "append[0] wire drifted (contract v1.0.0)")
        assertEquals(offAppend1, got[1], "append[1] wire drifted (contract v1.0.0)")
        assertEquals(offAppend2, got[2], "append[2] wire drifted (contract v1.0.0)")
        assertEquals(offFinalize, got[3], "finalize wire drifted (contract v1.0.0)")
    }

    @Test
    fun goldenWireDefaultViaConfig() {
        val got = stream(FluxSession.newWithConfig(libDefaultConfig(false)))
        assertEquals(offAppend0, got[0], "append[0] wire drifted via config (contract v1.0.0)")
        assertEquals(offAppend1, got[1], "append[1] wire drifted via config (contract v1.0.0)")
        assertEquals(offAppend2, got[2], "append[2] wire drifted via config (contract v1.0.0)")
        assertEquals(offFinalize, got[3], "finalize wire drifted via config (contract v1.0.0)")
    }

    @Test
    fun goldenWireBlockData() {
        val got = stream(FluxSession.newWithConfig(libDefaultConfig(true)))
        assertEquals(onAppend0, got[0], "append[0] blockData wire drifted (contract v1.0.0)")
        assertEquals(onAppend1, got[1], "append[1] blockData wire drifted (contract v1.0.0)")
        assertEquals(onAppend2, got[2], "append[2] blockData wire drifted (contract v1.0.0)")
        assertEquals(onFinalize, got[3], "finalize blockData wire drifted (contract v1.0.0)")
    }
    ...
    fun goldenAllBlocksDefault() {
        ...
        session.finalize()
        assertEquals(offAllBlocks, session.allBlocks(), "allBlocks wire drifted (contract v1.0.0)")
    }

    @Test
    fun resetRestartsFreshFromZero() {
        // Canonical first-chunk output of a fresh parser (heading committed as id 0).
        val fresh = FluxSession().append("# Two\n\n")
    ...
    fun resetPreservesConfig() {
        val session = FluxSession.newWithConfig(libDefaultConfig(true))
