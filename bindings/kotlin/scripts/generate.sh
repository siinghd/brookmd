#!/usr/bin/env bash
#
# Regenerate the Kotlin uniffi bindings for flux-md-ffi and stage them into the
# `fluxmd` library module. Idempotent: run it after any change to the FFI crate
...
CRATE_DIR="$(cd "$KOTLIN_DIR/../../crates/flux-md-ffi" && pwd)"
PKG_PATH="io/github/siinghd/fluxmd"
OUT_KT_DIR="$KOTLIN_DIR/fluxmd/src/main/kotlin"
...
GEN_KT="$STAGING/$PKG_PATH/flux_md_ffi.kt"
[ -f "$GEN_KT" ] || fail "generation produced no $PKG_PATH/flux_md_ffi.kt"

mkdir -p "$OUT_KT_DIR/$PKG_PATH"
cp "$GEN_KT" "$OUT_KT_DIR/$PKG_PATH/flux_md_ffi.kt"
echo "done — wrote $OUT_KT_DIR/$PKG_PATH/flux_md_ffi.kt"
