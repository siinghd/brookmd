...
final class WireGoldenTests: XCTestCase {
    ...
    /// A `FluxConfig` at every setter's library default plus the block-data toggle.
    private func libDefaultConfig(blockData: Bool) -> FluxConfig {
        FluxConfig(
            gfmAutolinks: false,
            gfmAlerts: false,
            gfmTagfilter: false,
    ...
    private func stream(_ session: FluxSession) -> [String] {
        chunks.map { session.append(chunk: $0) } + [session.finalize()]
    }

    func testGoldenWireDefaultBare() {
        let got = stream(FluxSession())
        XCTAssertEqual(got[0], offAppend0, "append[0] wire drifted (contract v1.0.0)")
        XCTAssertEqual(got[1], offAppend1, "append[1] wire drifted (contract v1.0.0)")
        XCTAssertEqual(got[2], offAppend2, "append[2] wire drifted (contract v1.0.0)")
        XCTAssertEqual(got[3], offFinalize, "finalize wire drifted (contract v1.0.0)")
    }

    func testGoldenWireDefaultViaConfig() {
        let got = stream(FluxSession.newWithConfig(config: libDefaultConfig(blockData: false)))
        XCTAssertEqual(got[0], offAppend0, "append[0] wire drifted via config (contract v1.0.0)")
        XCTAssertEqual(got[1], offAppend1, "append[1] wire drifted via config (contract v1.0.0)")
        XCTAssertEqual(got[2], offAppend2, "append[2] wire drifted via config (contract v1.0.0)")
        XCTAssertEqual(got[3], offFinalize, "finalize wire drifted via config (contract v1.0.0)")
    }

    func testGoldenWireBlockData() {
        let got = stream(FluxSession.newWithConfig(config: libDefaultConfig(blockData: true)))
        XCTAssertEqual(got[0], onAppend0, "append[0] blockData wire drifted (contract v1.0.0)")
        XCTAssertEqual(got[1], onAppend1, "append[1] blockData wire drifted (contract v1.0.0)")
        XCTAssertEqual(got[2], onAppend2, "append[2] blockData wire drifted (contract v1.0.0)")
        XCTAssertEqual(got[3], onFinalize, "finalize blockData wire drifted (contract v1.0.0)")
    }
    ...
    func testGoldenAllBlocksDefault() {
        ...
        _ = session.finalize()
        XCTAssertEqual(session.allBlocks(), offAllBlocks, "allBlocks wire drifted (contract v1.0.0)")
    }

    func testResetRestartsFreshFromZero() {
        let fresh = FluxSession().append(chunk: "# Two\n\n")
    ...
    func testResetPreservesConfig() {
        let session = FluxSession.newWithConfig(config: libDefaultConfig(blockData: true))
