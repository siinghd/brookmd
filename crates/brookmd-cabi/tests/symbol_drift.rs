//! Symbol-drift tripwire: keeps `include/brook_md.h`, the Rust `#[no_mangle]`
...
//!  2. **Header parse** — the C header is scanned naively for `brook_…(` function
...
//!     on it must expose exactly the `brook_` symbols in [`EXPORTS`]. Skipped
...
/// header and the compiled library against. Keep in sync with `include/brook_md.h`
/// and the `#[no_mangle]` functions in `src/lib.rs`.
const EXPORTS: &[&str] = &[
    "brook_session_new",
    "brook_session_new_with_config",
    "brook_session_append",
    "brook_session_finalize",
    "brook_session_all_blocks",
    "brook_session_reset",
    "brook_session_retained_bytes",
    "brook_session_buffer_len",
    "brook_session_free",
    "brook_string_free",
    "brook_wire_version",
];
...
fn export_addresses() -> [*const (); 11] {
    ...
    let new_: extern "C" fn() -> *mut BrookSession = brook_session_new;
    let new_cfg: extern "C" fn(*const c_char) -> *mut BrookSession = brook_session_new_with_config;
    let append: extern "C" fn(*mut BrookSession, *const u8, usize) -> *mut c_char = brook_session_append;
    let finalize: extern "C" fn(*mut BrookSession) -> *mut c_char = brook_session_finalize;
    let all_blocks: extern "C" fn(*mut BrookSession) -> *mut c_char = brook_session_all_blocks;
    let reset: extern "C" fn(*mut BrookSession) = brook_session_reset;
    let retained: extern "C" fn(*mut BrookSession) -> u64 = brook_session_retained_bytes;
    let buffer_len: extern "C" fn(*mut BrookSession) -> u64 = brook_session_buffer_len;
    let free: extern "C" fn(*mut BrookSession) = brook_session_free;
    let string_free: extern "C" fn(*mut c_char) = brook_string_free;
    let version: extern "C" fn() -> *const c_char = brook_wire_version;
    [
        new_ as *const (),
        new_cfg as *const (),
        append as *const (),
...
/// Naively scan C source for `brook_<ident>(` function tokens (identifier chars are
...
fn header_declares_exactly_the_exports() {
    let header = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("include/brook_md.h");
    ...
    assert_eq!(
        declared,
        exports_set(),
        "header (include/brook_md.h) drifted from EXPORTS: \
...
fn find_cdylib() -> Option<PathBuf> {
    ...
    let names = ["libbrook_md_cabi.so", "libbrook_md_cabi.dylib", "brook_md_cabi.dll"];
...
/// Parse `brook_` dynamic symbols out of `nm -D --defined-only` output (stripping a
/// possible leading `_`, and ignoring mangled Rust symbols, which don't match).
fn brook_symbols_via_nm(lib: &PathBuf) -> Option<BTreeSet<String>> {
    ...
    for line in text.lines() {
        ...
        if let Some(name) = line.split_whitespace().last() {
            ...
            if name.starts_with("brook_") && name.bytes().all(|b| b.is_ascii_lowercase() || b == b'_')
            {
...
fn dynamic_symbols_match_exports() {
    ...
    let Some(syms) = brook_symbols_via_nm(&lib) else {
