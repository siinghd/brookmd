...
fn speculative_link_golden_sweep() {
    ...
    for md in inert_prefixes {
        ...
        assert!(
            html.contains("<a data-brook-pending=\"\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Link text Here</a>"),
...
fn speculative_inert_shape_no_href() {
    ...
    for md in [
        ...
        let html = streamed_open(md);
        assert!(
            html.contains("<a data-brook-pending=\"\" target="),
...
fn speculative_security_javascript_scheme_suppressed() {
    ...
    assert!(open.contains("<a data-brook-pending=\"\" target="), "inert pending <a> mid-stream: {open}");
...
fn speculative_reference_links_untouched() {
    ...
    assert!(streamed_open("[t]").contains("<a data-brook-pending=\"\" target="));
    assert!(!streamed_open("[t] (").contains("data-brook-pending"), "settled shortcut is literal");
    ...
    assert!(
        html.contains("[t]") && html.contains("<a data-brook-pending=\"\" target="),
...
fn speculative_title_midstream_stays_pending() {
    ...
    assert!(
        html.contains("<a data-brook-pending=\"\" target=") && !html.contains("href="),
...
fn speculative_bracketed_dest_edges() {
    ...
    assert!(streamed_open("[a](<partial").contains("<a data-brook-pending=\"\" target="));
    assert_parity("[a](<url>");
    assert!(streamed_open("[a](<url>").contains("<a data-brook-pending=\"\" target="));
    assert_parity("[a](<br<");
...
fn speculative_convergence_node_stability() {
    ...
    assert!(s1.contains("<a data-brook-pending=\"\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Link</a>"));
...
fn speculative_cache_commit_safety_long_prefix() {
    ...
    assert!(streamed.contains("<a data-brook-pending=\"\" target="), "inert pending <a> present: ...{}", &streamed[streamed.len().saturating_sub(120)..]);
...
/// under the sanitizer (the brookmd JS default) and `unsafe_html` — in plain
