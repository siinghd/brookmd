//! `flux-md-cabi`: a plain **C-ABI** wrapper over [`flux_md_core`], designed for
//! consumption from Dart (`dart:ffi`) by the `flux_md` Flutter package, and by any
//! other language with a C FFI. The hand-written C header is `include/flux_md.h`.
//!
//! It exposes the streaming markdown parser as an opaque [`FluxSession`] whose
//! `append`/`finalize`/`all_blocks` functions return the **JSON wire strings**
//! specified in `../../flux-md-core/WIRE.md` (wire contract v1.0.0). Those strings
//! are produced through the pure [`flux_md_core::wire`] helpers, so they are
//! byte-identical to the WASM/JS boundary and to the uniffi (`flux-md-ffi`) layer
//! by construction — a native renderer decodes exactly the same bytes.
//!
//! This mirrors `flux-md-ffi`'s uniffi `FluxSession`, but with no bindgen: the
//! surface here is a small set of `#[no_mangle] extern "C"` functions.
//!
//! ## Ownership and safety contract (see also `include/flux_md.h`)
//!
//! - A `*mut FluxSession` is created by [`flux_session_new`] /
//!   [`flux_session_new_with_config`] and **must** be released with
//!   [`flux_session_free`] exactly once. Passing it to `free` twice is UB, exactly
//!   like C `free()`.
//! - Every function returning `*mut c_char` transfers ownership of a
//!   NUL-terminated, UTF-8 JSON string to the caller; free it with
//!   [`flux_string_free`] exactly once. Do **not** free it with libc `free`.
//! - [`flux_wire_version`] returns a pointer to a static string; it must **not**
//!   be freed.
//! - A `FluxSession` is **not** internally synchronized. A single session must not
...
//!   [`flux_session_reset`] (or free the session) before feeding more input.
...
use flux_md_core::wire::{blocks_to_json, patch_to_json, WirePatch};
use flux_md_core::{Block, StreamParser};

/// Wire contract version this crate emits (see `../../flux-md-core/WIRE.md`).
/// Returned by [`flux_wire_version`]. A `\0`-terminated static C string.
const WIRE_VERSION: &CStr = c"1.0.0";

/// Per-stream parser configuration, deserialized from the JSON object passed to
/// [`flux_session_new_with_config`]. Field names are the **snake_case** keys of
/// `FluxConfig` in `flux-md-ffi` (and of `ParserConfig` on the JS side, mapped to
...
/// `flux-md-ffi`'s uniffi `FluxConfig`): GFM autolinks and alerts default **on**
...
impl Default for FluxConfig {
    fn default() -> Self {
        // Mirrors flux-md-ffi's uniffi `FluxConfig` defaults: autolinks + alerts
...
/// Build a [`StreamParser`] from a [`FluxConfig`], applying the same
/// `StreamParser::set_*` calls, in the same order, as `flux-md-ffi`'s
...
fn build_parser(config: &FluxConfig) -> StreamParser {
...
/// from (kept so [`flux_session_reset`] can rebuild an identical parser, mirroring
/// the JS worker's free-and-recreate). `config = None` means the bare
/// [`flux_session_new`] constructor (library defaults).
///
/// Opaque to C: only ever handled through a `*mut FluxSession` pointer.
...
/// / the WASM `new FluxParser()`: GFM autolinks and alerts **off**, raw HTML
/// escaped, block data off).
///
/// Returns an owning pointer the caller must release with [`flux_session_free`],
/// or NULL if allocation panicked.
#[no_mangle]
pub extern "C" fn flux_session_new() -> *mut FluxSession {
    catch_unwind(|| {
        Box::into_raw(Box::new(FluxSession { parser: StreamParser::new(), config: None }))
    })
...
/// whose keys are `FluxConfig`'s snake_case field names (unknown keys ignored;
/// missing keys take library defaults — autolinks/alerts on, the rest off/None).
///
/// Returns an owning pointer (free with [`flux_session_free`]), or **NULL** if
...
#[no_mangle]
pub extern "C" fn flux_session_new_with_config(config_json: *const c_char) -> *mut FluxSession {
    catch_unwind(|| {
        ...
        let config: FluxConfig = match serde_json::from_str(json) {
            Ok(c) => c,
            Err(_) => return ptr::null_mut(), // invalid / non-object JSON
        };
        let parser = build_parser(&config);
        Box::into_raw(Box::new(FluxSession { parser, config: Some(config) }))
    })
    .unwrap_or(ptr::null_mut())
}
...
/// [`flux_string_free`] — or NULL if `s` is NULL, `chunk` is NULL with `len != 0`,
/// or a panic was caught.
///
/// # Safety
/// `s`, if non-NULL, must be a live pointer from a `flux_session_new*` call;
/// `chunk`, if non-NULL, must point to at least `len` readable bytes.
#[no_mangle]
pub extern "C" fn flux_session_append(
    s: *mut FluxSession,
    chunk: *const u8,
    len: usize,
) -> *mut c_char {
...
/// `s`, if non-NULL, must be a live pointer from a `flux_session_new*` call.
#[no_mangle]
pub extern "C" fn flux_session_finalize(s: *mut FluxSession) -> *mut c_char {
    catch_unwind(AssertUnwindSafe(|| {
        // SAFETY: see `flux_session_append`.
        let session = match unsafe { s.as_mut() } {
            Some(session) => session,
            None => return ptr::null_mut(),
        };
...
/// / caught panic. Free with [`flux_string_free`].
///
/// # Safety
/// `s`, if non-NULL, must be a live pointer from a `flux_session_new*` call.
#[no_mangle]
pub extern "C" fn flux_session_all_blocks(s: *mut FluxSession) -> *mut c_char {
    catch_unwind(AssertUnwindSafe(|| {
        // SAFETY: see `flux_session_append`.
...
/// `s`, if non-NULL, must be a live pointer from a `flux_session_new*` call.
#[no_mangle]
pub extern "C" fn flux_session_reset(s: *mut FluxSession) {
...
/// `s`, if non-NULL, must be a live pointer from a `flux_session_new*` call.
#[no_mangle]
pub extern "C" fn flux_session_retained_bytes(s: *mut FluxSession) -> u64 {
    catch_unwind(AssertUnwindSafe(|| {
        // SAFETY: see `flux_session_append`.
...
/// `s`, if non-NULL, must be a live pointer from a `flux_session_new*` call.
#[no_mangle]
pub extern "C" fn flux_session_buffer_len(s: *mut FluxSession) -> u64 {
    catch_unwind(AssertUnwindSafe(|| {
        // SAFETY: see `flux_session_append`.
        match unsafe { s.as_ref() } {
...
/// Release a session created by `flux_session_new*`. No-op if `s` is NULL. Calling
/// it twice on the same pointer is undefined behavior, exactly like C `free()`.
///
/// # Safety
/// `s` must be NULL or a pointer returned by a `flux_session_new*` call that has
/// not already been freed.
#[no_mangle]
pub extern "C" fn flux_session_free(s: *mut FluxSession) {
    if s.is_null() {
        return;
    }
    let _ = catch_unwind(AssertUnwindSafe(|| {
...
/// Free a string returned by `flux_session_append` / `finalize` / `all_blocks`.
...
#[no_mangle]
pub extern "C" fn flux_string_free(ptr: *mut c_char) {
    if ptr.is_null() {
        return;
    }
    let _ = catch_unwind(AssertUnwindSafe(|| {
...
/// The wire contract version string (`"1.0.0"`). Returns a pointer to a **static**
/// NUL-terminated string that must **NOT** be freed and lives for the program's
/// lifetime.
#[no_mangle]
pub extern "C" fn flux_wire_version() -> *const c_char {
    WIRE_VERSION.as_ptr()
}
