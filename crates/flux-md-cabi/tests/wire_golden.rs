...
//! **byte-identical** wire (WIRE.md v1.0.0) to `flux-md-core`'s WASM/JS boundary
//! and to `flux-md-ffi`'s uniffi layer.
//!
//! The document, chunking, and golden strings are copied verbatim from
//! `crates/flux-md-ffi/tests/wire_golden.rs` (which copied them from
//! `crates/flux-md-core/tests/wire_envelope_golden.rs`, crate 0.20.3). Those
...
//! reproduce that exact config two ways: the bare `flux_session_new` (equivalent
//! to `StreamParser::new()`), and `flux_session_new_with_config` with a JSON
...
/// `String`, and free it with `flux_string_free` (the correct deallocator). Panics
...
fn take_string(ptr: *mut c_char) -> String {
    ...
    flux_string_free(ptr);
    owned
}

/// Append a `&str` chunk through the length-based C export.
fn append(s: *mut FluxSession, chunk: &str) -> String {
    take_string(flux_session_append(s, chunk.as_ptr(), chunk.len()))
}

/// Stream [`CHUNKS`] through a session, returning each append patch (in order)
/// followed by the finalize patch.
fn stream(s: *mut FluxSession) -> Vec<String> {
    let mut out: Vec<String> = CHUNKS.iter().map(|c| append(s, c)).collect();
    out.push(take_string(flux_session_finalize(s)));
    out
}

#[test]
fn golden_wire_default_bare() {
    // Bare `flux_session_new` == `StreamParser::new()` == the wasm `new FluxParser()`.
    let s = flux_session_new();
    let got = stream(s);
...
fn golden_wire_default_via_config() {
    ...
    let s = flux_session_new_with_config(cfg.as_ptr());
...
fn golden_all_blocks_default() {
    ...
    for c in CHUNKS {
        ...
        flux_string_free(flux_session_append(s, c.as_ptr(), c.len()));
    }
    flux_string_free(flux_session_finalize(s));
    assert_eq!(
        take_string(flux_session_all_blocks(s)),
        OFF_ALLBLOCKS,
        "allBlocks wire drifted (contract v1.0.0)"
    );
    flux_session_free(s);
}
...
fn new_with_config_rejects_bad_input() {
    assert!(flux_session_new_with_config(std::ptr::null()).is_null(), "NULL config → NULL");

    let bad = CString::new("{not valid json").unwrap();
    assert!(flux_session_new_with_config(bad.as_ptr()).is_null(), "invalid JSON → NULL");

    let non_object = CString::new("[1,2,3]").unwrap();
    assert!(flux_session_new_with_config(non_object.as_ptr()).is_null(), "non-object JSON → NULL");
}
...
fn null_session_is_tolerated() {
    let null = std::ptr::null_mut::<FluxSession>();
    assert!(flux_session_append(null, b"x".as_ptr(), 1).is_null(), "append(NULL) → NULL");
    assert!(flux_session_finalize(null).is_null(), "finalize(NULL) → NULL");
    assert!(flux_session_all_blocks(null).is_null(), "all_blocks(NULL) → NULL");
    assert_eq!(flux_session_retained_bytes(null), 0, "retained_bytes(NULL) → 0");
    assert_eq!(flux_session_buffer_len(null), 0, "buffer_len(NULL) → 0");
    flux_session_reset(null); // no-op, must not crash
    flux_session_free(null); // no-op, must not crash
    flux_string_free(std::ptr::null_mut()); // no-op, must not crash
}
...
fn null_chunk_handling() {
    let s = flux_session_new();
    let empty = flux_session_append(s, std::ptr::null(), 0);
    assert!(!empty.is_null(), "NULL chunk with len 0 is a valid empty append");
    flux_string_free(empty);
    assert!(
        flux_session_append(s, std::ptr::null(), 5).is_null(),
...
fn reset_restarts_fresh_from_zero() {
    let fresh_s = flux_session_new();
    let fresh = append(fresh_s, "# Two\n\n");
    flux_session_free(fresh_s);
...
fn metrics_track_input() {
    let s = flux_session_new();
    assert_eq!(flux_session_buffer_len(s), 0, "empty session has an empty buffer");
    flux_string_free(flux_session_append(s, b"# Title\n\n".as_ptr(), 9));
    assert_eq!(flux_session_buffer_len(s), 9, "buffer_len counts retained source bytes");
    assert!(flux_session_retained_bytes(s) > 0, "retained_bytes includes buffer + rendered html");
    flux_session_reset(s);
    assert_eq!(flux_session_buffer_len(s), 0, "reset() clears the buffer");
    flux_session_free(s);
}
...
fn no_interior_nul_in_output() {
    ...
    let ptr = flux_session_append(s, with_nul.as_ptr(), with_nul.len());
...
#[test]
fn wire_version_is_1_0_0() {
    let ptr = flux_wire_version();
    assert!(!ptr.is_null());
    // SAFETY: static NUL-terminated string; never freed.
    let v = unsafe { CStr::from_ptr(ptr) }.to_str().unwrap();
    assert_eq!(v, "1.0.0", "wire contract version must match WIRE.md");
}
