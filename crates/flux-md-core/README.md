...
engine behind the [`flux-md`](https://www.npmjs.com/package/flux-md) npm package,
...
- `wasm` (default) — compiles the wasm-bindgen `FluxParser` glue used by the
  `flux-md` JS package.
...
```toml
[dependencies]
flux-md-core = { version = "0.20", default-features = false }
```

## Wire format

Blocks and patches serialize to a stable, language-agnostic JSON wire format —
see [WIRE.md](WIRE.md) (wire contract v1.0.0). Native consumers can produce the
same bytes as the WASM/JS boundary via `wire::patch_to_json` / `wire::blocks_to_json`.

## Links

- npm package: <https://www.npmjs.com/package/flux-md>
