...
//! `]`-at-EOF hold, ref phase, and the `data-flux-pending` marker.
//!
...
//!   - `data-flux-pending=""` is present exactly while the link is pending and
...
const PENDING: &str =
    "<a data-flux-pending=\"\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">";
...
fn assert_link_frames(make: fn() -> StreamParser, md: &str, flash: &str, href_frag: &str) {
    ...
    for (i, f) in fr.iter().enumerate() {
        ...
        let pending = f.contains("data-flux-pending");
        saw_pending |= pending;
        assert!(
    ...
    assert!(
        last.contains(href_frag) && !last.contains("data-flux-pending"),
...
fn paragraph_label_visible_from_first_char_no_bracket_flash() {
    ...
    for (i, f) in fr.iter().enumerate().take(close).skip(open + 1) {
        ...
        assert!(f.contains("data-flux-pending"), "not pending at frame {i}: {f}");
    }
    ...
    assert!(!done.contains("data-flux-pending"));
...
fn ref_style_unknown_downgrades_to_literal() {
    ...
    assert!(
        settled.contains("[thing][nope]") && !settled.contains("data-flux-pending"),
...
fn literal_brackets_downgrade_pinned() {
    ...
    for f in &fr[close + 1..] {
        assert!(!f.contains("data-flux-pending"));
    }
    assert_finalize_collapse(fresh, md);
}
...
fn footnote_candidates_never_speculate() {
    ...
    assert!(fr.iter().any(|f| f.contains("data-flux-pending")));
}
...
fn checkbox_lookalike_link_upgrades_at_paren() {
    ...
    assert!(done.contains("href=\"https://x.example/y\"") && !done.contains("data-flux-pending"));
    assert_finalize_collapse(fresh, md);
}
...
fn multiline_label_downgrades_at_newline() {
    ...
    let fr = frames(md);
    let nl = md.find('\n').unwrap();
    assert!(fr[nl - 1].contains("data-flux-pending")); // before the newline
...
fn oversized_label_downgrades_at_cap() {
    ...
    let fr = frames(&md);
    assert!(fr[400].contains("data-flux-pending"));
    let last = fr.last().unwrap();
    assert!(
        !last.contains("data-flux-pending") && last.contains("[aaa"),
