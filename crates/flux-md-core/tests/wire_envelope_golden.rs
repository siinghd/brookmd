//! Wire-format golden stability tests — **wire contract v1.0.0** (see `../WIRE.md`).
//!
...
fn golden_all_blocks_default() {
    ...
    assert_eq!(blocks_to_json(&all), OFF_ALLBLOCKS, "allBlocks wire drifted (contract v1.0.0)");
}
...
fn block_field_inventory() {
    ...
    assert_eq!(keys, expected, "Block field set changed — breaking wire change (contract v1.0.0)");
}
