//! `flux-md-ffi`: a [uniffi](https://mozilla.github.io/uniffi-rs/)-annotated
//! wrapper over [`flux_md_core`], designed for consumption by
//! [uniffi-bindgen-react-native (ubrn)](https://jhugman.github.io/uniffi-bindgen-react-native/).
//!
//! It exposes the streaming markdown parser as a [`FluxSession`] object whose
...
//!
//! This mirrors the `#[wasm_bindgen] FluxParser` glue in `flux-md-core`'s
//! `lib.rs` (gated behind its `wasm` feature): [`FluxSession::new`] is the bare
//! constructor (equivalent to `new FluxParser()`), and [`FluxConfig`] +
//! [`FluxSession::new_with_config`] mirror the per-stream configuration the JS
//! worker applies in `packages/flux-md/src/worker.ts` (`makeParser`).
...
///
/// SAFETY: this type is only ever reached through [`FluxSession::inner`], a
...
// SAFETY: see [`SendParser`] — access is fully serialized by `FluxSession`'s
...
impl FluxSession {
    ...
    #[uniffi::constructor]
    pub fn new_with_config(config: FluxConfig) -> Arc<Self> {
    ...
    pub fn append(&self, chunk: String) -> String {
        let mut p = self.inner.lock().expect("FluxSession parser mutex poisoned");
    ...
    pub fn all_blocks(&self) -> String {
        let p = self.inner.lock().expect("FluxSession parser mutex poisoned");
    ...
    pub fn retained_bytes(&self) -> u64 {
        self.inner.lock().expect("FluxSession parser mutex poisoned").0.retained_bytes() as u64
    }
    ...
    pub fn buffer_len(&self) -> u64 {
        self.inner.lock().expect("FluxSession parser mutex poisoned").0.buffer().len() as u64
    }
}
