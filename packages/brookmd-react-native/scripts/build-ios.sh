#!/usr/bin/env bash
#
# Build the brookmd-ffi Rust crate for iOS device + simulator, lipo the two
# simulator slices into one fat static lib, and assemble an XCFramework the
# podspec (brookmd-react-native.podspec) vendors.
#
...
XCFRAMEWORK="$PKG_DIR/ios/BrookMdFfi.xcframework"
