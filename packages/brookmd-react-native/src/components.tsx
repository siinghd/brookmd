...
// shape `brookmd`'s `htmlToReact` consumes) in which every HTML tag the core
...
import type { Block, Components, ListData, NestedBlock, TableData } from "brookmd/types";
import { htmlToReact, safeUrl } from "brookmd/html-to-react";
 ...
 * are typed `ComponentType<any>` (matching brookmd's `Components`), so both a real
...
export const RENDER_OPEN_BLOCK: unique symbol = Symbol("brookmd-rn.renderOpenBlock");

/**
 * Build the tag→primitive component map. Pass the result as `<BrookMarkdown
...
export function createComponents(primitives: RnPrimitives, theme: Theme): Components {
  ...
  const A = (p: P) => {
    const pending = "data-brook-pending" in p;
