// The JS backend that plugs the native (uniffi/JSI) parser into brookmd's
// existing streaming client. brookmd's browser build runs each stream's parser
// in a Web Worker; React Native has no Worker, so instead of a real thread we
// wrap brookmd's `WorkerCore` (the worker's message/readiness state machine,
// extracted precisely so it can run anywhere) in a synchronous, in-process
// `WorkerLike`. The pool + client on top are the SAME `BrookPool` / `BrookClient`
// the browser uses — only the transport differs.
//
// The native parser itself is dependency-injected: `createNativePool()` defaults
// to the ubrn-generated `BrookSession` (registered by the package entry via
...
// stack off-device.
import { BrookClient, BrookPool } from "brookmd/client";
import { WorkerCore, type ParserLike } from "brookmd/worker-core";
import type { FromWorker, ParserConfig, ToWorker, WorkerLike } from "brookmd/types";
...
function defaultMakeParser(config: ParserConfig | undefined): ParserLike {
  if (!nativeParserFactory) {
    throw new Error(
      "brookmd-react-native: native parser not registered. Import from the package " +
        "entry (`brookmd-react-native`) so the uniffi TurboModule is wired, or pass " +
...
/**
 * A synchronous, in-process `WorkerLike` backing one brookmd pool "worker".
 *
 ...
 * envelopes the browser worker posts over `postMessage`, so `BrookPool` /
 * `BrookClient` cannot tell the difference. One `NativeWorker` multiplexes many
...
/**
 * Build a `BrookPool` whose "workers" are in-process `NativeWorker`s. The cap is
 ...
 *   native `BrookSession`). Host tests pass a WASM-backed factory here.
 */
export function createNativePool(opts: { makeParser?: MakeParser } = {}): BrookPool {
  const makeParser = opts.makeParser ?? defaultMakeParser;
  return new BrookPool(() => new NativeWorker(makeParser), 1);
}

// The process-wide native pool every `createBrookClient()` shares unless given a
// pool or a custom `makeParser`.
let defaultNativePool: BrookPool | null = null;

/** The shared process-wide native pool (created lazily). */
export function getDefaultNativePool(): BrookPool {
...
/**
 * Convenience wrapper over `new BrookClient({ pool: createNativePool() })`. The
 * returned client behaves exactly like brookmd's browser client (`append`,
