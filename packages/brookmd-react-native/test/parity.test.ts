...
// NativeWorker → WorkerCore → the injected WASM parser → BrookClient store) with a
...
beforeAll(async () => {
  if (!haveWasm) {
    ...
    console.warn("[brookmd-react-native] brookmd WASM not built — run `bun run build:wasm`; skipping parity.");
    return;
  }
  Ctor = await loadWasm();
});
...
describe("streaming parity through the native shim", () => {
  test.skipIf(!haveWasm)("a chunk-by-chunk stream equals a one-shot parse", async () => {
    const client = new BrookClient({ pool: createNativePool({ makeParser: wasmMakeParser(Ctor) }), config: RICH_CONFIG });
    try {
