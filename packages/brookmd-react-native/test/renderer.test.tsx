...
beforeAll(async () => {
  if (!haveWasm) {
    ...
    console.warn("[brookmd-react-native] brookmd WASM not built — skipping wire-driven renderer tests.");
    return;
  }
  Ctor = await loadWasm();
...
describe("RN component map (unit, no WASM)", () => {
  ...
  test("a data-brook-pending link is subdued and NOT pressable", () => {
    const el = (comps.a as (p: unknown) => ReactElement)({ "data-brook-pending": "", children: "pending" });
    const rn = toRN(el) as RnNode;
    expect(rn.rn).toBe("Text");
