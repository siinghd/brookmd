// packages/brookmd/scripts/build.mjs
//
// Structure-preserving compiled-dist build for brookmd.
//
// Why this shape (NOT a single inlined bundle): brookmd's core design relies on
...
assert(statSync(path.join(distDir, "wasm/brook_md_core_bg.wasm")).isFile(), "wasm binary missing");
assert(statSync(path.join(distDir, "wasm/brook_md_core.js")).isFile(), "wasm glue missing");
...
// brookmd/server is the documented React-FREE entry — a non-React consumer must
// be able to import renderToString/parseToBlocks/initBrook without react installed.
...
// React server component lives in dist/server-react.js (brookmd/server/react).
...
console.log("brookmd dist build OK ->", distDir);
