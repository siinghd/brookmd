...
function BrookMarkdownFromStream(props: BrookMarkdownProps) {
  const client = useBrookStream(props.stream, {
    config: props.streamConfig,
    onError: props.onStreamError,
  });
  return <BrookMarkdownFromClient {...props} client={client} />;
}
...
function BrookMarkdownImpl(props: BrookMarkdownProps) {
  if (props.stream != null && props.client == null) {
    return <BrookMarkdownFromStream {...props} />;
  }
  if (props.client == null) {
    throw new Error("<BrookMarkdown>: pass either a `client` or a `stream` prop.");
  }
  return <BrookMarkdownFromClient {...(props as BrookMarkdownProps & { client: BrookClient })} />;
}

export const BrookMarkdown = memo(BrookMarkdownImpl);
