// Inspect a downstream Vite build's output and prove brookmd's worker + wasm
// graph survived as SEPARATE, non-inlined artifacts. Pass the output dir as argv[2]
// (run.sh points it at web/dist-brook, the real consumer build of brook-entry.ts
// against the BUILT brookmd dist).
...
const glue = [...contents].filter(([, s]) => s.includes("__wbindgen") || s.includes("brook_md_core_bg"));
