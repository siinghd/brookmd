#!/usr/bin/env bash
# End-to-end consumer smoke test for brookmd's COMPILED dist.
#
...
#   4. Node ESM, worker-free: resolve brookmd/server through the real exports map
#      and render (string + RSC) — no network
#   5. Browser: Vite-build the in-repo consumer (web/src/brook-entry.ts imports
#      brookmd/element + /client + /styles.css) against the BUILT dist and assert
...
PKG="$(cd "$HERE/../.." && pwd)"                 # packages/brookmd
...
mv "$WORK/node_modules/package" "$WORK/node_modules/brookmd"
...
( cd "$WEB" && ./node_modules/.bin/vite build --config vite.brook.config.ts >/dev/null 2>&1 )

echo "== 6. assert separate worker chunk + non-inlined wasm =="
node "$HERE/assert-vite-output.mjs" "$WEB/dist-brook"
