...
function mountWith(html: string, options: Parameters<typeof mountBrookMarkdown>[2]) {
  const { client, worker } = makeClient();
  const container = win.document.createElement("div");
  const handle = mountBrookMarkdown(client, container as unknown as HTMLElement, { batch: false, ...options });
  drive(client, worker, patch([para(1, html)], []));
  const block = container.querySelector(".brook-block-paragraph")!;
...
test("DOM: a committed decorated block's node is frozen across tail patches", () => {
  const { client, worker } = makeClient();
  const container = win.document.createElement("div");
  mountBrookMarkdown(client, container as unknown as HTMLElement, { batch: false, decorators: pctDecorators });
  // Commit id=1 (one match) + open tail id=2 (no match).
  drive(client, worker, patch([para(1, "<p>up 10%</p>")], [para(2, "<p>tw</p>", true)]));
  const committedNode = container.querySelector(".brook-block-paragraph")!;
  ...
  const stillCommitted = container.querySelectorAll(".brook-block-paragraph")[0];
...
test("DOM: streamed-then-committed output equals one-shot committed output", () => {
  ...
  const streamedBlock = container.querySelector(".brook-block-paragraph")!;
