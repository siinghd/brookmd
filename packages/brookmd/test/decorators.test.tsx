...
import { BrookMarkdown, blocksEqual, __resetUnstableWarnings } from "../src/react";
...
function fakeClient(blocks: Block[]) {
  return { subscribe: () => () => {}, getSnapshot: () => blocks } as unknown as BrookClient;
}
const renderStatic = (html: string, props: Record<string, unknown>) =>
  renderToStaticMarkup(
    createElement(BrookMarkdown, { client: fakeClient([para(1, html, false)]), ...props }),
  );
...
test("a committed block is decorated exactly once across many streamed tail patches", async () => {
  ...
  await mount(createElement(BrookMarkdown, { client, decorators }));
...
test("an unstable decorators prop identity fires a one-time dev warning", async () => {
  ...
  try {
    ...
    const { root } = await mount(createElement(BrookMarkdown, { client, decorators: first }));
    ...
    await act(async () => {
      root.render(createElement(BrookMarkdown, { client, decorators: second }));
    });
    ...
    await act(async () => {
      root.render(createElement(BrookMarkdown, { client, decorators: third }));
    });
...
test("streamed char-by-char output equals one-shot output with decorators on", async () => {
  ...
  const oneClient = (() => {
    const w = new FakeWorker();
    const c = new BrookClient({ pool: new BrookPool(() => w, 1) });
    c.append("");
    return { c, w, sid: (w.sent[0] as { streamId: number }).streamId };
  })();
  const oneMount = await mount(createElement(BrookMarkdown, { client: oneClient.c, decorators }));
  await act(async () => {
    oneClient.w.fire({
      type: "patch",
  ...
  const strMount = await mount(createElement(BrookMarkdown, { client: strClient.c, decorators }));
