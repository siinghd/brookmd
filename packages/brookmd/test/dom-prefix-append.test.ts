...
function fullRebuildInnerHTML(b: Block): string {
  ...
  const handle = mountBrookMarkdown(client, container, { batch: false, highlightCode: false });
  drive(client, worker, patch([], [b]));
  const html = (container.querySelector(".brook-md") as HTMLElement).innerHTML;
  handle.destroy();
  return html;
}
...
test("PREFIX-APPEND: paragraph-sibling growth splices the suffix and matches the full rebuild", () => {
  withAppendSpy((count) => {
    ...
    const root = container.querySelector(".brook-md") as HTMLElement;
...
test("PREFIX-APPEND: sanitizer present always falls back (suffix can't be independently sanitized)", () => {
  withAppendSpy((count) => {
    ...
    const handle = mountBrookMarkdown(client, container, { batch: false, sanitize });
    const root = container.querySelector(".brook-md") as HTMLElement;
