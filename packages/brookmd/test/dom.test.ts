...
// requestAnimationFrame, so `mountBrookMarkdown` with the default batch falls to
...
// The fake worker has no real stream; BrookClient.append() posts a message we
...
test("streaming code block renders plain body; on commit it highlights (once)", () => {
  ...
  // Open: plain body (raw `.brook-code-body > div`, no highlight token spans),
  ...
  expect(node.querySelector(".brook-code-body > div")).not.toBeNull(); // raw body
  expect(node.querySelector(".brook-code-body > pre")).toBeNull(); // not highlighted yet
  ...
  const pre = node.querySelector(".brook-code-body > pre code");
  expect(pre).not.toBeNull();
  expect(node.querySelector(".brook-code-streaming-pill")).toBeNull();
  expect(node.querySelector(".brook-code-copy")).not.toBeNull();
...
test("sanitize is applied to the open/speculative tail", () => {
  ...
  expect(node.className).toContain("brook-open");
...
test("destroy() unsubscribes (later patches don't mutate the DOM) and removes root", () => {
  ...
  handle.destroy();
  expect(container.querySelector(".brook-md")).toBeNull(); // root removed
...
test("stickToBottom keeps a scroll-snap anchor pinned last", () => {
  ...
  client.append("");
  const container = document.createElement("div");
  const handle = mountBrookMarkdown(client, container, { batch: false, stickToBottom: true });
  const root = container.querySelector(".brook-md")!;
  ...
  expect(stillLast.className).toContain("brook-bottom-anchor");
...
test("virtualize sets content-visibility on closed blocks, never on the open tail", () => {
  ...
  client.append("");
  const container = document.createElement("div");
  const handle = mountBrookMarkdown(client, container, { batch: false, virtualize: true });
  const root = container.querySelector(".brook-md")!;
...
test("block-kind override replaces the whole block and receives props", () => {
  ...
  expect(node.querySelector(".brook-code-block")).toBeNull();
