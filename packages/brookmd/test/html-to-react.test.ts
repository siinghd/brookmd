...
import { BrookMarkdown, blocksEqual } from "../src/react";
...
// ---------------------------------------------------------------------------
// BrookMarkdown dispatch (fake client, no worker)
// ---------------------------------------------------------------------------
...
test("no components prop → byte-identical innerHTML wrapper, parser untouched", () => {
  const out = render(createElement(BrookMarkdown, { client: fakeClient([para("<p>hi</p>")]) }));
...
test("#5: open block + components → override applies to the streaming tail too (parsed once)", () => {
  ...
  expect(out).toContain("brook-open"); // still flagged as the open tail
...
test("CodeBlock with no override → dedicated highlighter renderer", () => {
  ...
  expect(out).toContain("brook-code-block");
  expect(out).toContain("brook-code-lang");
});
...
test("CodeBlock (closed) renders a copy button with an accessible label", () => {
  ...
  const out = render(createElement(BrookMarkdown, { client: fakeClient([b]) }));
  expect(out).toContain("brook-code-copy");
  expect(out).toContain('aria-label="Copy code"');
  expect(out).toContain('type="button"');
  // No streaming pill on a closed block.
  expect(out).not.toContain("brook-code-streaming-pill");
});
...
test("CodeBlock (open / streaming) hides the copy button until close", () => {
  ...
  const out = render(createElement(BrookMarkdown, { client: fakeClient([b]) }));
  expect(out).toContain("brook-code-streaming-pill");
  expect(out).not.toContain("brook-code-copy");
...
test("CodeBlock + tag-level code override bypasses the highlighter", () => {
  ...
  expect(out).not.toContain("brook-code-block"); // highlighter bypassed
...
test("Alert with no override renders the GitHub-compatible HTML", () => {
  ...
  expect(out).toContain("brook-block-alert");
});
...
test("virtualize wraps closed blocks with content-visibility, sized per kind", () => {
  const out = render(createElement(BrookMarkdown, { client: fakeClient([para("<p>hi</p>")]), virtualize: true }));
...
test("virtualize uses the right per-kind estimate (code block ~300px)", () => {
  ...
  const out = render(createElement(BrookMarkdown, { client: fakeClient([cb]), virtualize: true }));
...
test("virtualize never defers the streaming tail (open or speculative)", () => {
  const open = render(createElement(BrookMarkdown, { client: fakeClient([para("<p>partial", true)]), virtualize: true }));
  expect(open).not.toContain("content-visibility");
  const spec = block({ kind: { type: "Paragraph" }, html: "<p>x</p>", open: false, speculative: true });
  const out = render(createElement(BrookMarkdown, { client: fakeClient([spec]), virtualize: true }));
...
test("stickToBottom renders a scroll-snap sentinel as the last child", () => {
  const out = render(createElement(BrookMarkdown, { client: fakeClient([para("<p>hi</p>")]), stickToBottom: true }));
  expect(out).toContain("scroll-snap-align:end");
  // Sentinel is the final element of .brook-md.
  expect(out.endsWith('class="brook-bottom-anchor"></div></div>')).toBe(true);
});

test("no stickToBottom → no sentinel (default unchanged)", () => {
  const out = render(createElement(BrookMarkdown, { client: fakeClient([para("<p>hi</p>")]) }));
  expect(out).not.toContain("scroll-snap-align");
  expect(out).not.toContain("brook-bottom-anchor");
});
...
test("safeStyle drops url()/position:fixed/expression but keeps safe declarations", () => {
  // brookmd's own table-alignment style must survive.
