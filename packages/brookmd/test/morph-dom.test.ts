...
test("morphOpenBlocks: morphed open-tail subtree equals the innerHTML-rebuilt subtree across a streaming sequence", () => {
  ...
  const hA = mountBrookMarkdown(a.client, cA, { batch: false, morphOpenBlocks: true });
  const hB = mountBrookMarkdown(b.client, cB, { batch: false });
  const rootA = cA.querySelector(".brook-md")!;
  const rootB = cB.querySelector(".brook-md")!;
...
test("morphOpenBlocks default OFF: open-tail node is rebuilt on growth (no behavior change)", () => {
  ...
  client.append("");
  const container = document.createElement("div");
  const handle = mountBrookMarkdown(client, container, { batch: false }); // default
  const root = container.querySelector(".brook-md")!;
...
test("morphOpenBlocks does not apply to code blocks (highlighter path unaffected)", () => {
  ...
  expect(v1.querySelector(".brook-code-streaming-pill")).not.toBeNull();
  ...
  expect((root.children[0] as HTMLElement).querySelector(".brook-code-streaming-pill")).not.toBeNull();
