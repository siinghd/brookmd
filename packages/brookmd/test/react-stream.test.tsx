...
import { BrookMarkdown, useBrookStream, useBrookMarkdownString } from "../src/react";
...
test("<BrookMarkdown stream> pipes an AsyncIterable: append per chunk, then finalize once", async () => {
  const appendSpy = spyOn(BrookClient.prototype, "append");
  ...
  try {
    ...
    await mount(createElement(BrookMarkdown, { stream: gen() }));
    await act(async () => {
...
test("useBrookStream destroys its owned client on unmount, and only then", async () => {
  let captured: BrookClient | null = null;
  function Probe({ stream }: { stream: AsyncIterable<string> }) {
    captured = useBrookStream(stream);
...
test("<BrookMarkdown client> NEVER destroys the caller-owned client on unmount", async () => {
  ...
  const { root } = await mount(createElement(BrookMarkdown, { client }));
  await act(async () => {
    root.unmount();
  });
...
test("toggling between `client` and `stream` props does not violate the Rules of Hooks", async () => {
  ...
  const caller = new BrookClient({ pool });
  caller.append("");
  async function* gen() {
    yield "a";
  }
  ...
  await act(async () => {
    root.render(createElement(BrookMarkdown, { client: caller }));
  });
  await act(async () => {
    root.render(createElement(BrookMarkdown, { stream: gen() }));
  });
  await act(async () => {
    await tick();
  });
...
test("React StrictMode double-mount never double-finalizes and never throws", async () => {
  ...
  try {
    ...
    await mount(createElement(StrictMode, null, createElement(BrookMarkdown, { stream: gen() })));
    await act(async () => {
      await tick();
    });
...
test("#5: a supplied sanitize runs on component-rendered blocks (closes the bypass)", async () => {
  ...
  const { host } = await mount(createElement(BrookMarkdown, { client, components, sanitize }));
  await act(async () => {
    created[0].fire({
      type: "patch",
      streamId: sid,
...
test("useBrookMarkdownString diffs a growing string into appends and finalizes when done", async () => {
  const appendSpy = spyOn(BrookClient.prototype, "append");
  const finalizeSpy = spyOn(BrookClient.prototype, "finalize");
  try {
    function Probe({ content, streaming }: { content: string; streaming: boolean }) {
      useBrookMarkdownString(content, { streaming });
      return createElement("div", null, "ok");
    }
...
test("useBrookMarkdownString: omitting `streaming` leaves it open; `streaming:false` finalizes", async () => {
  ...
  try {
    function Probe({ content, streaming }: { content: string; streaming?: boolean }) {
      useBrookMarkdownString(content, streaming === undefined ? undefined : { streaming });
      return createElement("div", null, "ok");
    }
