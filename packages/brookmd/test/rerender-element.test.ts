...
import { defineBrookMarkdown } from "../src/element";
...
// The `<brook-markdown>` custom element's public surface.
...
beforeAll(() => {
  ...
  // No requestAnimationFrame: mountBrookMarkdown falls to synchronous sync, so
...
test("ATTR-NOOP: a same-value config setAttribute does not rebuild the self-owned client (fix #1)", () => {
  ...
  const rootBefore = el.querySelector(".brook-md");
  ...
  expect(el.querySelector(".brook-md")).toBe(rootBefore); // SAME root node (no remount)
...
test("PROP-NOOP: re-assigning the same components/sanitize identity does not remount; a new identity does (fix #2)", () => {
  ...
  el.components = components as unknown as BrookEl["components"];
  el.sanitize = sanitize as unknown as BrookEl["sanitize"];
  ...
  el.components = components2 as unknown as BrookEl["components"];
  const rootAfter = el.querySelector(".brook-md");
