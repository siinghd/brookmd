...
import { BrookMarkdown, useBrookStream, blocksEqual } from "../src/react";
...
// react-7: USEBROOKSTREAM-STABLE — re-rendering with the SAME stream identity but a
...
test("react-7: same stream + new onError does not re-pipe or abort, and the latest onError wins", async () => {
  ...
  const pipeSpy = spyOn(BrookClient.prototype, "pipeFrom").mockImplementation(function (
    this: BrookClient,
    _source,
...
test("react-render-probe: committed block fires onRenderMetrics once; open tail fires per patch", async () => {
  ...
  await mount(createElement(BrookMarkdown, { client, onRenderMetrics }));
