...
// BrookMarkdown component has no `batch` prop, so dom.ts batches via rAF and only
...
//
// src/vue.ts is a THIN lifecycle glue over mountBrookMarkdown: it mounts once on
...
// not tear down and rebuild the .brook-md root on every parent re-render.
//
// We mount BrookMarkdown inside a wrapper that owns a counter ref it bumps M
// times (each bump forces a parent re-render). The .brook-md root captured right
...
test("vue-no-mountcount: unrelated parent re-renders never remount the renderer root", async () => {
  ...
  const wrapper = vue.defineComponent({
    setup() {
      ...
      // parent; BrookMarkdown's props stay identical across renders.
      return () =>
        vue.h("div", { "data-count": counter.value }, [
          vue.h(adapter.BrookMarkdown, { client }),
        ]);
    },
  });
  ...
  // re-render via nextTick. mountBrookMarkdown must have run exactly once: the
  ...
  for (let i = 0; i < M; i++) {
    ...
    expect(host.querySelector(".brook-md")).toBe(root);
  }
...
test("vue-tail-finegrain: useTailBlockId tracks the open tail; committed nodes keep identity", async () => {
  ...
  const wrapper = vue.defineComponent({
    setup() {
      ...
      readTail = () => tail.value;
      return () => vue.h(adapter.BrookMarkdown, { client });
    },
  });
  const app = vue.createApp(wrapper);
  app.mount(host);
