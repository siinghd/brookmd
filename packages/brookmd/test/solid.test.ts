...
test("mountSolid renders the client snapshot into the container", () => {
  ...
  const props: BrookMarkdownProps = { client, batch: false };
...
test("forwards MountOptions (stickToBottom/virtualize) through to the renderer", () => {
  ...
  const props: BrookMarkdownProps = { client, batch: false, stickToBottom: true, virtualize: true };
  mountSolid(() => props, container, cleanups.register);
...
test("cleanup runs handle.destroy (root removed) and never calls client.destroy", () => {
  ...
  expect(container.querySelector(".brook-md")).not.toBeNull();
...
// --------------------------------------------------------------------------
// createBrookMarkdownString — the controlled-string helper.
//
// We test the reactivity-free core `setupBrookMarkdownString` directly (the same
...
// We spy on BrookClient.prototype.setContent with a NO-OP so nothing spawns a
...
test("createBrookMarkdownString feeds content on first effect (done=false when streaming omitted)", () => {
  const setContentSpy = spyOn(BrookClient.prototype, "setContent").mockImplementation(() => {});
  const destroySpy = spyOn(BrookClient.prototype, "destroy").mockImplementation(() => {});

  const effects = makeEffectRunner();
  const cleanups = makeCleanups();
  const client = setupBrookMarkdownString(() => "# hi", undefined, effects.register, cleanups.register);

  expect(client).toBeInstanceOf(BrookClient);
...
test("createBrookMarkdownString re-feeds on content growth and finalizes when streaming flips false", () => {
  const setContentSpy = spyOn(BrookClient.prototype, "setContent").mockImplementation(() => {});
  const destroySpy = spyOn(BrookClient.prototype, "destroy").mockImplementation(() => {});
...
test("createBrookMarkdownString reads config ONCE in the body (immutable) and OWNS the client (cleanup destroys)", () => {
  const setContentSpy = spyOn(BrookClient.prototype, "setContent").mockImplementation(() => {});
  const destroySpy = spyOn(BrookClient.prototype, "destroy").mockImplementation(() => {});
  ...
  const effects = makeEffectRunner();
  const cleanups = makeCleanups();
  setupBrookMarkdownString(() => "x", getOptions, effects.register, cleanups.register);
...
test("createBrookMarkdownString (public) wires Solid's createEffect/onCleanup", () => {
