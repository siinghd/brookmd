...
import { BrookClient } from "../src/client";
import { BrookMarkdown } from "../src/react";

/**
 * SSR-safety harness (in-process).
 *
 * What this proves: importing the brookmd entrypoints, constructing a `BrookClient`,
 * and React-server-rendering each `<BrookMarkdown>` mode does NOT throw when no
 * browser globals exist (`window`/`document`/`Worker`/… all undefined). The
 * original bug was `new BrookClient()` eagerly calling `pool.acquire()` →
 * `new Worker(...)` in its constructor, throwing "Worker is not defined" on the
 * server (and during a React SSR render via `useBrookStream`'s
 * `useState(() => new BrookClient(...))` initializer).
 *
 ...
 * React (which does not cache `document` for server rendering) + the brookmd modules
...
// --------------------------------------------------------------------------
// 1. Import-safety: the brookmd entrypoints load without a browser env. (Framework
...
test("brookmd entrypoints import without a browser env", async () => {
...
// --------------------------------------------------------------------------
// 2. new BrookClient() is construct-safe + worker-free derived reads.
// --------------------------------------------------------------------------

test("new BrookClient() does not throw on the server and getSnapshot() === []", () => {
  withServerEnv(() => {
    const c = new BrookClient(); // MUST NOT throw "Worker is not defined"
...
// --------------------------------------------------------------------------
// 3. React renderToString — both <BrookMarkdown client/> and <BrookMarkdown stream/>.
//    The stream path is the ORIGINAL repro: useBrookStream's
//    `useState(() => new BrookClient(...))` initializer runs during the render
//    pass and used to throw "Worker is not defined".
// --------------------------------------------------------------------------

test("React renderToString(<BrookMarkdown client/>) is worker-free and stable", () => {
  withServerEnv(() => {
    const client = new BrookClient(); // construct-safe (lazy acquire)
    const html1 = renderToString(createElement(BrookMarkdown, { client }));
    const html2 = renderToString(createElement(BrookMarkdown, { client }));
    expect(html1).toBe(html2); // stable (empty snapshot both times)
    expect(html1).toContain('class="brook-md"');
    client.destroy(); // never-acquired destroy: no throw, no phantom release
  });
});

test("React renderToString(<BrookMarkdown stream/>) does not throw", () => {
  withServerEnv(() => {
    async function* gen() {
      yield "# hi\n";
    }
    ...
    const html = renderToString(createElement(BrookMarkdown, { stream: gen() }));
    expect(html).toContain('class="brook-md"');
  });
});
...
test("server snapshot equals the initial client snapshot (hydration parity)", () => {
  const serverSnap = withServerEnv(() => new BrookClient().getSnapshot());
  const clientSnap = new BrookClient().getSnapshot();
