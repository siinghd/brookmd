...
// actual BrookParser, its serde shape, or the setGfm*/setComponentTags binding.
...
beforeAll(async () => {
  if (!haveWasm) return;
  const glue = "../src/wasm/brook_md_core.js"; // variable specifier → resolved at runtime, not collection
...
function newParser(): any {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const p = new BrookParser() as any;
  return new Proxy(p, {
    get(target, prop) {
      const v = target[prop];
