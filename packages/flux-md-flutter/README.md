# flux_md (Flutter / Dart) — EXPERIMENTAL

Dart/Flutter bindings for **flux-md**, an incremental, streaming-aware markdown
parser. They wrap the plain C ABI in [`crates/flux-md-cabi`](../../crates/flux-md-cabi)
(over the `flux-md-core` Rust engine) with hand-written `dart:ffi` bindings.

> [!WARNING]
> **This package is EXPERIMENTAL and demand-driven.**
> The Rust C-ABI layer (`flux-md-cabi`) is unit-tested and CI-built for Android,
...
- `FluxSession` — one streaming-parse session. `append(String) -> String`,
  `finalize()`, `allBlocks()`, `reset()`, `retainedBytes()`, `bufferLen()`,
  `dispose()`.
- `FluxConfig` — per-stream options (GFM autolinks/alerts default on, everything
  else off), serialized to the JSON config the C ABI expects.
- `FluxSession.wireVersion()` — the wire-contract version string (currently
  `1.0.0`).

`append`/`finalize` return the **JSON wire strings** defined by
[`WIRE.md`](../../crates/flux-md-core/WIRE.md) — byte-identical to the WebAssembly/JS
...
```dart
import 'package:flux_md/flux_md.dart';

final session = FluxSession(config: const FluxConfig(blockData: true));
try {
...
| --- | --- | --- |
| Android | `libflux_md_cabi.so` | one per ABI in `jniLibs/<abi>/`; `cargo-ndk` cross-build |
| iOS | (statically linked) | link the `staticlib` slice / XCFramework into the app; symbols resolve from the process |
| macOS | `libflux_md_cabi.dylib` | `aarch64`/`x86_64-apple-darwin` |
| Linux | `libflux_md_cabi.so` | host `cargo build --release` |
| Windows | `flux_md_cabi.dll` | host `cargo build --release` |

Build the crate from `crates/flux-md-cabi`:

```bash
# Desktop (host):
cargo build --release          # -> target/release/libflux_md_cabi.{so,dylib,dll}
...
```
lib/flux_md.dart       public API (FluxSession, FluxConfig)
...
pubspec.yaml           name: flux_md, publish_to: none
```

## License

MIT, same as the rest of flux-md.
