/// EXPERIMENTAL Dart/Flutter bindings for the flux-md streaming markdown parser.
///
/// Feed markdown chunks with [FluxSession.append]; each call returns a JSON
/// `Patch` string as specified by the flux-md wire contract (WIRE.md v1.0.0). The
...
///
/// The native library (`libflux_md_cabi`) is built from `crates/flux-md-cabi`; see
...
/// Open the flux-md native library for the current platform.
///
/// - Android / Linux: `libflux_md_cabi.so`
/// - macOS: `libflux_md_cabi.dylib`
/// - Windows: `flux_md_cabi.dll`
/// - iOS: statically linked into the app binary, so symbols resolve from the
///   process itself ([ffi.DynamicLibrary.process]).
ffi.DynamicLibrary _openLibrary() {
  const base = 'flux_md_cabi';
  ...
  throw UnsupportedError('flux_md: no native library mapping for this platform');
}

/// The process-wide, lazily-resolved binding table. All sessions share it.
final FluxBindings _bindings = FluxBindings(_openLibrary());
...
class FluxSession {
  FluxSession._(this._handle);

  final ffi.Pointer<FluxSessionHandle> _handle;
  bool _disposed = false;

  /// Create a session. With no [config], the parser uses library defaults
  /// (autolinks/alerts on, raw HTML escaped, block data off).
  factory FluxSession({FluxConfig? config}) {
    final handle = config == null ? _bindings.sessionNew() : _newWithConfig(config);
    if (handle == ffi.nullptr) {
      throw StateError('flux_md: failed to create session (invalid config?)');
    }
    return FluxSession._(handle);
  }

  static ffi.Pointer<FluxSessionHandle> _newWithConfig(FluxConfig config) {
  ...
  /// the correct deallocator (`flux_string_free`, never libc free).
  String _takeString(ffi.Pointer<ffi.Char> ptr) {
    if (ptr == ffi.nullptr) {
      throw StateError('flux_md: native call returned NULL');
    }
    try {
  ...
  void _checkAlive() {
    if (_disposed) {
      throw StateError('flux_md: session used after dispose()');
    }
  }
}
