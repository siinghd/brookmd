#!/usr/bin/env bash
#
# Build the flux-md-ffi Rust crate for the Android device ABIs and stage each
...
for i in "${!TARGETS[@]}"; do
  ...
  cp "$CRATE_DIR/target/$target/release/libflux_md_ffi.so" "$JNI_LIBS/$abi/libflux_md_ffi.so"
done
