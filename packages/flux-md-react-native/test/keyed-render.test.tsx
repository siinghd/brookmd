...
import { getParseCount, htmlToReact, resetParseCount } from "flux-md/html-to-react";
import type { Block, Components } from "flux-md/types";
...
import { makeFluxMarkdown, __resetUnstableWarnings } from "../src/FluxMarkdown";
...
beforeAll(async () => {
  if (!haveWasm) {
    ...
    console.warn("[flux-md-react-native] flux-md WASM not built — skipping keyed-render tests.");
    return;
  }
  Ctor = await loadWasm();
...
describe("keyed vs whole-html equivalence (closed blocks)", () => {
  test("the keyed dispatcher survives the {...defaults, ...overrides} merge", () => {
    // FluxMarkdown builds `{ ...createComponents(...), ...props.components }`; the
...
describe("dev unstable-prop warning", () => {
  test("warns once when the `theme` prop changes identity across renders", () => {
    const FM = makeFluxMarkdown({ primitives: hostPrimitives, useColorScheme: () => "light" });
    ...
    const EMPTY: never[] = [];
    const fakeClient = { subscribe: () => () => {}, getSnapshot: () => EMPTY } as unknown as FluxClient;
