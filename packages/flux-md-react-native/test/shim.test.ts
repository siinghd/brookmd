...
import { createFluxClient, createNativePool, __resetDefaultNativePool } from "../src/native-pool";
...
describe("native transport shim", () => {
  test("whenReady() resolves once the in-process worker signals ready", async () => {
    const client = new FluxClient({ pool: createNativePool({ makeParser: fakeMakeParser() }) });
  ...
  test("createFluxClient wires the shared native pool", async () => {
    const client = createFluxClient({ makeParser: fakeMakeParser() });
    client.append("hi");
    client.finalize();
    await client.whenReady();
    await settle();
  ...
  test("an unregistered native parser surfaces a clear, non-fatal error", async () => {
    ...
    const client = new FluxClient({ pool: createNativePool(), onError: (e) => errors.push(e) });
    client.append("boom");
    await client.whenReady();
    await settle();
