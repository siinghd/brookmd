...
for (const k of ["initFlux", "initFluxSync", "isFluxReady", "parseToBlocks", "renderToString", "FluxMarkdownStatic"]) {
...
const rsc = renderToString(createElement(m.FluxMarkdownStatic, { content: "## sub *em*" }));
...
console.log("ok  - FluxMarkdownStatic (RSC):", JSON.stringify(rsc.slice(0, 60)));
