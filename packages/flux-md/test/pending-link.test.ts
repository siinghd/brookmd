...
// arriving) with `data-flux-pending=""` and NO href; when the `)` lands the
...
const PENDING =
  '<p>Check the <a data-flux-pending="" target="_blank" rel="noopener noreferrer nofollow">Earnings Call</a></p>';
const FINAL =
...
test("data-flux-pending survives the React attr allowlist into props and markup", () => {
  const tree = htmlToReact(PENDING, {});
  const a = findTag(tree, "a");
  expect(a).not.toBeNull();
  const props = a!.props as Record<string, unknown>;
  expect(props["data-flux-pending"]).toBe("");
  expect("href" in props).toBe(false); // inert: no href while pending

  const out = render(tree);
  expect(out).toContain('data-flux-pending=""');
  expect(out).not.toContain("href");
  expect(out).toContain("Earnings Call");
});

test("a valueless data-flux-pending (boolean attr form) also reaches the markup", () => {
  const tree = htmlToReact("<p><a data-flux-pending>label</a></p>", {});
  const a = findTag(tree, "a");
  expect((a!.props as Record<string, unknown>)["data-flux-pending"]).toBe(true);
  // React stringifies a boolean data-* value; the CSS presence selector
  // `a[data-flux-pending]` matches either serialization.
  expect(render(tree)).toContain("data-flux-pending");
});
...
test("a re-render from the completed html drops the marker and carries the href", () => {
  ...
  expect(out).not.toContain("data-flux-pending");
});

test("a tag-level `a` override receives data-flux-pending so it can style pending links", () => {
  let got: unknown = "unset";
  const components = {
    a: (p: Record<string, unknown>) => {
      got = p["data-flux-pending"];
...
test("DOM renderer: the marker lands on the live anchor and a URL-complete patch clears it", () => {
  ...
  expect(a.hasAttribute("data-flux-pending")).toBe(true);
  ...
  expect(a.hasAttribute("data-flux-pending")).toBe(false);
...
test("morphOpenBlocks: the reused anchor node sheds the stale marker and gains the href", () => {
  ...
  expect(a0.hasAttribute("data-flux-pending")).toBe(true);
  ...
  expect(a1.hasAttribute("data-flux-pending")).toBe(false);
